...
const navItems = [
  ...
  { id: 'images', label: 'Packet Scanner', icon: Globe },
  { id: 'manual-inspection', label: 'Manual Inspect', icon: Layers },
        ...
        >
          <div className="bg-[#0A0E1A] rounded-lg p-3 border border-white/5">
            ...
            <p className="text-xs text-cyan-400 mt-1">
              {session.summary?.total_packets?.toLocaleString() ?? '0'} packets            </p>
          </div>
        </motion.div>
      )}
        ...
        <div className="hidden md:flex items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-400">
              <span className="text-white font-medium">{session.summary?.total_packets?.toLocaleString() ?? '0'}</span> packets            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-400">
              <span className="text-white font-medium">{((session.summary?.total_bytes ?? 0) / 1024 / 1024).toFixed(2)}</span> MB            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-400">
              <span className="text-white font-medium">{session.summary?.duration_seconds ?? '0'}s</span> duration            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 rounded-lg border border-red-500/20">
            <span className="text-xs text-red-400 font-medium">
              {(session.summary?.critical_alerts ?? 0) + (session.summary?.high_alerts ?? 0)} threats
            </span>
          </div>
