...
export function TrafficTimeline({ summary }: TrafficTimelineProps) {
  ...
  const hasData = summary?.total_packets > 0 || summary?.duration_seconds > 0;
  ...
  const generateTimelineData = () => {
    ...
    for (let i = 0; i <= 20; i++) {
      ...
      const packets = Math.floor(total_packets / 20) + Math.floor(Math.random() * Math.max(1, total_packets / 40));
      const totalBytes = (summary.total_bytes ?? total_packets * 800) || 1;
      const bytes = Math.floor((totalBytes / 20) * (0.8 + Math.random() * 0.4));
